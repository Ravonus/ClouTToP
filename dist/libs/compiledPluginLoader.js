...
function testPluginRequire(filePath) {
    try {
...
export function loadPlugins(name, filePath) {
    const checkedFilePath = testPluginRequire(filePath);
    if (typeof checkedFilePath !== 'string')
        return checkedFilePath;
    const mainFilePath = path.join(__dirname, '../../', '.webpack/renderer/main_window/index.js');
    let fileString = fs.readFileSync(mainFilePath, 'utf-8');
    const foundPluginsString = fileString.match(/const plugins = {[^\/]+/);
    if (!foundPluginsString)
        return new Error('Issue finding plugins');
    const foundPluginBrackets = foundPluginsString[0].match(/{[^}]*/);
    if (!foundPluginBrackets)
        return new Error('Issue finding plugins brackets.');
